...
The twenty-ninth increment adds deterministic architecture-package production and the
first signed GitHub draft-release workflow. The standalone
`AetherSDR.ReleaseBuilder` executable is separate from the running web host and receives
one exact architecture asset directory, release/channel/compatibility declarations, a
new exact manifest destination, and one owner-only PKCS#8 ECDSA P-256 private-key file.
It requires exactly the four fixed role archives, rejects links, missing/extra files,
empty/oversized inputs, package metadata drift, invalid semantic transitions, unsafe key
permissions, unsupported key material, and existing output. It signs the canonical
version-1 payload and then submits the generated bytes, package digests, exact channel,
architecture, and public key back through the production `SignedReleaseManifestVerifier`.
The manifest is created atomically only after self-verification succeeds. Reports expose
no key path, private material, signature, package path, or checksum.

`build-github-release-assets.sh` publishes the web gateway plus independent watchdog,
broker, AetherRemote agent, and station engine for `linux-x64` and `linux-arm64` with
continuous-integration deterministic build settings. It normalizes generated modes,
uses sorted tar entries, numeric root ownership, one commit-derived timestamp, and
`gzip -n`. The current gateway and station-engine topology packages the same reviewed
web/watchdog tree, so those role archives are byte-identical. Both architectures undergo
the existing production web/watchdog TX string-count, transport-marker, forbidden-HIL,
and disabled-configuration inspection before signing. Both successful manifests and
eight packages are promoted together as ten read-only assets into one new output
directory; the private key is never copied.

The manual `draft-release.yml` workflow runs only from `main`, requests the protected
`release-signing` environment, reruns the complete production validation-only gate, and
requires `AETHERSDR_RELEASE_SIGNING_KEY_PKCS8_BASE64` plus an explicit reviewed key ID.
It writes the decoded key into a mode-0600 temporary file, builds the ten exact assets,
removes the key, rejects an existing release identity, and creates only a GitHub draft
at the exact workflow commit. Stable, Beta, and Pinned manifests use the same channel
contracts as the checker; Beta or prerelease semantic versions mark the GitHub draft as
a prerelease. The workflow never publishes the draft, deploys, installs, activates,
restarts services, connects to a radio, mutates a lease, sends a radio command, keys, or
performs live RF.

Automated checkpoint on 2026-08-05 for signed release artifact packaging: the static
release-automation boundary gate passed; all changed shell files passed syntax checks;
changed C# projects passed scoped format verification; `git diff --check` passed; and
the 13-project Release solution built with zero warnings and zero errors. The focused
release-builder suite passed 17/17. Web tests passed 1,561/1,561; independent-watchdog
tests passed 57/57; TX-HIL isolation tests passed 48/48; AetherRemote tests passed
70/70; and browser tests passed 135/135. The complete checkpoint covered 1,753 .NET
tests and 1,888 tests overall.

A real local build used one ephemeral owner-only P-256 key outside the repository and
produced exactly ten assets: four role packages plus one self-verified signed manifest
for each of `linux-x64` and `linux-arm64`. The gateway and station-engine role archives
were byte-identical for each architecture as required by the current topology, both
architecture publish trees passed production TX/HIL artifact inspection, and no private
key marker appeared in any output. A second full build from the same commit, key, and
source epoch produced all eight package archives byte-for-byte identically; both signed
manifests retained identical canonical payloads and signature metadata.

The final validation-only release was `20260805-085523-flexweb-validation`. It reran the
release automation gate and all 1,888 tests, rebuilt the complete solution, published
the normal production web/watchdog artifacts, proved the build-time
`ReleaseManifestBuilder` type was absent, retained the reviewed web/watchdog TX command
string shape, and started the independent watchdog empty and Disarmed. No GitHub draft
...
- M8G implementation evidence on 2026-08-12: the 14-project Release build passes
